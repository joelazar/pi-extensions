...
describe("jsonSchemaToZodShape", () => {
	...
	it("lists free-form object params through the Agent SDK MCP server", async () => {
		const shape = jsonSchemaToZodShape({
			type: "object",
		...
		const server = createSdkMcpServer({
			name: "t",
			version: "1",
			tools: [{ name: "x", description: "x", inputSchema: shape, handler: async () => ({ content: [] }) }],
		});
		const listTools = server.instance.server._requestHandlers.get("tools/list");
		const { tools } = await listTools({ method: "tools/list", params: {} }, {});

		assert.equal(tools[0].inputSchema.properties.spec.type, "object");
		assert.deepEqual(z.object(shape).parse({ spec: { a: 1 } }), { spec: { a: 1 } });
	});
});
