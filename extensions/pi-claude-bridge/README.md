...
Copied from [w-winter/dot314](https://github.com/w-winter/dot314/tree/main/extensions/pi-claude-bridge), itself a fork of [Eli Dickinson's `@vanillagreen/pi-claude-bridge`](https://github.com/vanillagreencom/kendex/tree/main/pi-extensions/pi-claude-bridge). Unlike dot314, pi loads `src/index.ts` directly; there is no committed bundle.

![Response from Claude through the bridge](assets/bridge-demo.png)

## Install

Run `npm install` at the repo root, then load it for one run:

```bash
pi -e ./extensions/pi-claude-bridge/src/index.ts
```
